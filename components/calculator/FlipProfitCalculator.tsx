...
// Fix & Flip program term range is 12-24 months per spec
const termOptions = [12, 18, 24];
...
export default function FlipProfitCalculator() {
  ...
  // Midpoint of Dalton's 9–11.5% range
    ...
    try {
      await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          message: `Calculator review request: Purchase ${formatCurrency(
            purchasePrice,
          )}, Rehab ${formatCurrency(rehabBudget)}, ARV ${formatCurrency(
            arv,
          )}, Loan ${formatCurrency(loanAmount)}, Profit ${formatCurrency(
